          ...
          $no++;
            ...
            echo (" 
              ...
              <tr>
                ...
                <td><a href='".base_url()."index.php/barang/struk_penjualan/".$x->grup_penjualan."' target='blank' class='btn btn-info btn-xs btn-block'>Detail</a>
                  <button type='button' class='btn btn-danger  btn-xs btn-block' onclick='batalkan_trx(\"$x->grup_penjualan\")' >Batalkan</button>
                </td>                                
              </tr>
          ");
...
function batalkan_trx(grup_penjualan)
{
  if(confirm("Anda yakin membatalkan trx "+grup_penjualan+" ? "))
  {
    $.post("<?php echo base_url()?>index.php/barang/go_batalkan_trx",{grup_penjualan:grup_penjualan},function(){
      eksekusi_controller('<?php echo base_url()?>index.php/barang/lap_penjualan_batalkan/?mulai=<?php echo date( 'Y-m-d', strtotime(' -1 day' ))?>&selesai=<?php echo date('Y-m-d',strtotime('+1 days'));?>&id_cabang=<?php echo $this->session->userdata('id_cabang')?>','Transaksi Penjualan');return false;
    })
  }
